...
namespace appeufiz.Views
{
    public partial class NewItemPage : ContentPage
    {
        ...
        async void LoadLocal(string itemId)
        {
            try
            {
                int id = Convert.ToInt32(itemId);
                // Retrieve the note and set it as the BindingContext of the page.
                Item olocal = await App.Database.GetLocalAsync(id);
                BindingContext = olocal;
            }
            catch (Exception)
            {
        ...
        {
            var olocal = (Item)BindingContext;

            if (!string.IsNullOrWhiteSpace(olocal.Nome))
            {
                await App.Database.SaveLocalAsync(olocal);
            }
        ...
        {
            var olocal = (Item)BindingContext;
            await App.Database.DeleteLocalAsync(olocal);
